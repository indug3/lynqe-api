...
const Accounts = require("../accounts/account.js");
...
router.post("/register", async (req, res) => {
    try {
        ...
        const newUser = await Accounts.create({ name, email, password: hashedPassword });
...
router.get("/all", authenticateToken, authorizeAdmin, async (req, res) => {
    try {
        const users = await Accounts.findAll({ attributes: { exclude: ["password"] } });
...
router.get("/profile", authenticateToken, async (req, res) => {
    try {
        const user = await Accounts.findByPk(req.user.id, {
